...
def get_html_zhidao(url):
    ...
    soup_zhidao = BeautifulSoup(requests.get(url=url, headers=headers).content, "html.parser")
...
def get_html_bingwd(url):
    headers = {'User-Agent':'Mozilla/5.0 (X11; U; Linux i686)Gecko/20071127 Firefox/2.0.0.11'}
    soup_bingwd = BeautifulSoup(requests.get(url=url, headers=headers).content, "html.parser")
...
def get_html_baidu(url):
    headers = {'User-Agent':'Mozilla/5.0 (X11; U; Linux i686)Gecko/20071127 Firefox/2.0.0.11'}
    soup_baidu = BeautifulSoup(requests.get(url=url, headers=headers).content.decode('utf-8'), "html.parser")
...
def get_html_bing(url):
    ...
    soup_bing = BeautifulSoup(requests.get(url=url, headers=headers).content.decode('utf-8'), "html.parser")
